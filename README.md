...
> **当前进度：P2 已完成** —— 在 P1(工具设置/输入解封装/码率分析)基础上，新增
> ③ **H.264 逐帧逐宏块语法解析**：调用 JM 解码器生成 trace，解析为按帧组织的语法树，
> 每个字段附**中英文名、bit 位置、二进制、数值、含义与标准章节**。
> 路线图见文末，后续 P3–P5 实现帧预览叠加、原始数据联动、HEVC CU 级分析。
