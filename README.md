...
> **当前进度：P3 已完成** —— 在 P1(工具/输入/码率)、P2(语法解析)基础上，新增
> ④ **H.264 帧预览与分层叠加**：逐帧 step 预览，可开关叠加 **宏块及各类细分子块分类配色**
> (帧内 4×4/8×8/16×16、帧间 16×16/16×8/8×16/8×8 及子分割 8×4/4×8/4×4、Skip/Direct/PCM)、
> **运动矢量、帧内预测方向、QP 热力、帧间参考关系**。
> 路线图见文末，后续 P4–P5 实现原始数据联动、HEVC CU 级分析。
...
4. **④ 帧预览**（H.264）
   ...
   - 右侧「叠加图层」独立开关：**宏块/子块划分**（各类子块分类配色）、**QP 热力**、**运动矢量**、**帧内预测方向**、**帧间参考关系**。
   - 图例面板列出全部块类型颜色；信息面板给出帧类型/POC/QP 范围/子块统计。
   - 鼠标悬停任意子块显示其类型/尺寸/预测方向/QP/MV。

5. **⑤** 标签为 P4 功能占位，实现后自动可用。
